...
var WatcherTask = {
	...
	dependFiles: function (info) {
		...
		if (_.isFunction(dependFiles)) { // todo, 要对返回值做类型检查
			...
			var result = []
			dependFiles.forEach(function (dependFile) {
				if (_.isString(dependFile)) {
					result.push(dependFile)
					return
				}
				...
				while (true) {
					var match = dependFile.exec(code) // todo, 正则表达式必须是贪婪模式
					if (!match) break
		...
		return _.map(result, function (dependFile) { // todo, 进一步校验, 必须属于该工程的文件
			if (path.isAbsolute(dependFile)) {
				dependFile = path.relative(info.dirPath, dependFile)
			}

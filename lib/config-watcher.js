...
module.exports = {
	...
	_onChangeAdd: function (event) {
		return function (absolutePath) {
			logger.info('watcher config'.cyan, event, absolutePath)

			try {
				var watcherConfig = reload(absolutePath)
				this._config.add(absolutePath, watcherConfig)
			} catch (e) {
				logger.error('watcher config'.red, 'reload error')
				return
			}
	...
	_onDelete: function (absolutePath) {
		logger.info('watcher config'.cyan, 'delete', absolutePath)
	...
	start: function (ready) {
		this._watcher = chokidar.watch(this._configPath, {
			...
			.on('ready', function () {
				logger.info('watcher config'.cyan, 'scan over')
				ready()
			})

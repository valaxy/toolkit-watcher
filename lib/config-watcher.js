...
module.exports = {
	init: function (projectWatcher) {
		...
		this._configPath = u.normalizePath(path.join(projectWatcher.root(), CONFIG_DIR))
		this._projectWatcher = projectWatcher
	},

	_onChangeAdd: function (event) {
		return function (absolutePath) {
			logger.info('config', event, u.normalizePath(absolutePath))
			...
			try {
				...
				logger.error(null, 1, e.stack.grey)
				return
			}
